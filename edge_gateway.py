...
DEVICE_TOKEN = os.environ.get("HW_CLIENT_SECRET", "secret_door_01")

WEBCAM_INDEX = 0             # Índice de la cámara
MIN_DETECTION_CONF = 0.6     # Umbral de confianza de detección de MediaPipe
    ...
    while cap.isOpened():
        ...
        if results.detections:
            ...
            if auth_status not in ["GRANTED", "DENIED", "PROCESSING"] and (current_time - last_auth_time > COOLDOWN_TIME):
                ...
                # Renderizar frame temporal para feedback visual inmediato
                ...
                if face_img.size > 0:
                    ...
                    try:
                        print("\n🔍 Rostro detectado. Enviando solicitud de autenticación...")
                        ...
                        if response.status_code == 200 and response_data.get("authorization") == "GRANTED":
                            ...
                            # Simulación de activación del relé físico/puerta
